...
func (r *SearchRepository) Upsert(ctx context.Context, entry models.SearchEntry) error {
	...
	return r.db.WithContext(ctx).Exec(`
		...
		ON CONFLICT (id) DO UPDATE SET
			product_id = EXCLUDED.product_id, search_tokens = EXCLUDED.search_tokens,
			category_ids = EXCLUDED.category_ids, lat = EXCLUDED.lat,
			long = EXCLUDED.long, geo_hash = EXCLUDED.geo_hash,
			available = EXCLUDED.available, updated_at = NOW(), deleted_at = NULL
