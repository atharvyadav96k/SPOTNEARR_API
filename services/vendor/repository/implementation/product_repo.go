...
func (p *ProductRepository) DeleteProduct(ctx context.Context, productID uint, bizID uint) error {
	return p.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		// Decrement token frequencies for removed product.
		...
		// Find all inventory products for this product to generate delete outbox events.
		var invIDs []uint
		if err := tx.Model(&models.InventoryProduct{}).
		...
		for _, invID := range invIDs {
			payload, err := json.Marshal(models.OutboxPayload{
				EventType:    "delete",
				InvProductID: invID,
			})
			if err != nil {
			...
			if err := tx.Create(&models.SearchSyncOutbox{
				EventType: "delete",
				Payload:   payload,
			}).Error; err != nil {
		...
		// Soft delete inventory products (DB cascade only applies to hard deletes).
		if len(invIDs) > 0 {
			if err := tx.Where("product_id = ?", productID).Delete(&models.InventoryProduct{}).Error; err != nil {
				return err
			}
		}
