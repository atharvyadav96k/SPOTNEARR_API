...
func (s *StoreRepository) GetStoreByBusinessId(ctx context.Context, bizID uint) ([]vendordb.Store, error) {
	...
	db := s.db.WithContext(ctx).
		...
		Where("businesses.id = ? AND businesses.deleted_at IS NULL", bizID).
		Scan(&results)
	if db.Error != nil {
		return nil, db.Error
	}
...
func (s *StoreRepository) CreateStoreByBusinessId(ctx context.Context, store *vendordb.Store) error {
...
func (s *StoreRepository) UpdateStoreByBusinessId(ctx context.Context, store vendordb.Store) (vendordb.Store, error) {
	var existing vendordb.Store
	if err := s.db.WithContext(ctx).
		Where("id = ? AND business_id = ?", store.ID, store.BusinessID).
		First(&existing).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return vendordb.Store{}, fmt.Errorf("store with ID %d not found for business %d", store.ID, store.BusinessID)
		}
		return vendordb.Store{}, err
	}
	...
	if err := s.db.WithContext(ctx).Model(&existing).Updates(updates).Error; err != nil {
		return vendordb.Store{}, fmt.Errorf("failed to update store: %w", err)
	}
